...
=== Verifying Dockerfiles (linter)

The Dockerfile-lint is a rule based 'linter' for verifying Dockerfiles. The rules are used to check syntax and best practice options for things such as:
...
The input rules are defined via a set of yaml files. At the time of this writing, there are a number of templates from base to automated build configurations.
...
For the next section, we will assume that the linter will be installed manually via npm using the following commands:
...
In the sample_rules directory, there are some included templates for openshift and an example base template. These are the rules we will be referencing below.

==== Using the linter

Upon evokation of the CLI version of the linter, some sample output may include:

```
-> bin/dockerfile_lint -f ~/Documents/manage-iq/Dockerfile.2 -r sample_rules/basic_rules.yaml
```
If you examine the command listed above you'll notice the inclusion of the yaml ruleset: sample_rules/basic_rules.yaml.

If you take a look inside this file you'll see various rules defined as such:
...
---
  ...
  -
   ...
   message: "yum clean all is not used" # Message to display
   description: "the yum cache will remain in this layer making the layer unnecessarily large" # Longer message to display
   reference_url:
...
WARNING: No 'USER' instruction. The process(es) within the container may run as root and RUN instructions my be run as root.
Reference -> https://docs.docker.com/reference/builder/#user
...
Let's revisit the rules example from above:
...
The include section would allow you to define your own yaml file and place it into the sample_rules directory. This would allow for execution of additional rules in addition to the file it was called from.

In the example above, the basic_rules.yaml rules are processed in addition to recommended_label_rules.yaml.
