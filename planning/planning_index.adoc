...
Currently, the most popular and a reasonable way to pass credentials and secrets in a container is specifying them as **environment variables** at container runtime. You as a consumer of such an image don't expose any of your sensitive data publicly.
...
Containers running through Kubernetes can take advantage of the **secret** object type to store sensitive data such as passwords or tokens. To learn how to create and access these, refer to the http://kubernetes.io/docs/user-guide/secrets/[Kubernetes User Guide].
