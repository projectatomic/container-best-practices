...
Images that have no parent are called base images. Every Docker image has its own root filesystem with an operating system installed. So when you want to create a new container, it either has to be based on an image that actually provides an operating system or you will need to create this layer in your image.
