...
This set of rules is a basic catchall of your typical Dockerfile. Things such as yum cache clean up and command execution etiquette are checked.
...
* OSBS Template - dockerfile_lint/sample_rules/osbs.yaml

NOT SURE IF THIS IS GOING TO GO OR NOT - TODO TODO TODO

==== How do I use the linter?

Evocation of the CLI version of the linter may look like this:
...
Inside this file is how the rules are defined:
...
```
In the event of an error being thrown from the linter, the programs exit code is reflected appropriately. This allows for quick and automated testing as what is informational and what needs to be addressed immediately.

An except from the rules shows how includes are defined:
