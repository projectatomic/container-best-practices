...
The rules are implememented using regular expressions, run on one instruction of the dockerfile at a time. The rule file has 4 sections, *a profile section*, *a line rule section and a required instructions section.*
...
This section contains rules that must be run on a given instruction in the dockerfile. The line rules do the bulk of the dockerfile parsing.
