...
to bind mount the host's _/etc/localtime_ with the container's _/etc/localtime_.  We use the `ro` flag to ensure the container
...
// TBD: === Where to store related components
...
==== Use systemd
Using systemd to start your application is great if your application is service-oriented (like httpd).
...
//TBD: === Techniques for deploying or starting images from a host
...
multiple containers, you will need to ensure that this storage is set up for the user.

==== Persistent Storage for Containers: Data Volumes

Docker defines link:https://docs.docker.com/engine/admin/volumes/volumes/[persistent storage] in two ways.

. Data volumes
. Data volume containers

However at present, the use of data volumes
...
advantages that they can be shared and reused for one or more containers.  Moreover, a data volume will
persist even if the associated container is deleted.

Data volumes must be explicitly created and preferably should be named to provide it with a meaningful name.  You can
manually create a data volume with the `docker volume create` command.
...
We recommend that the data volume be created through the use of the INSTALL label.  The INSTALL label
...
to allows users to obtain, review, and possibly retain those logs.  The flexibility of a container environment
...
handle your log files.  Each of the aforementioned solutions has its pros and cons.
...
its log files.  Often the logging service will coordinate logging with journald but nevertheless it too is a service and
...
Depending on the host distribution, log messages will now be in the host's journald and subsequently in
...
directory between host and container using the `-v` flag in your RUN label.
...
your image is run.  This can be achieved with the following switch to `docker run`.
