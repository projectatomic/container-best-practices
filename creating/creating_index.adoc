...
The Dockerfile-lint is a rule based 'linter' for verifying Dockerfiles. The rules are used to check syntax and best practice options for things such as 'Was yum clean up evoked after a package installation?' or 'In the RUN section did the writer link commands via semicolons or double ampersands?'. The input rules are called via a set of yaml files. At the time of this writing, there are a number of templates from base to automated build configurations. 
...
ERROR: Required LABEL name/key 'Architecture' is not defined. 
